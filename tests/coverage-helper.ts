...
export const generateMarkdownReport = (report: CoverageReport): string => {
	...
	markdown += `| ------ | ----- | ------- | -------- |\n`;
	markdown += `| Lines | ${report.summary.lines.total} | ${
		report.summary.lines.covered
	} | ${report.summary.lines.percentage.toFixed(2)}% |\n`;
	markdown += `| Statements | ${report.summary.statements.total} | ${
		report.summary.statements.covered
	} | ${report.summary.statements.percentage.toFixed(2)}% |\n`;
	markdown += `| Functions | ${report.summary.functions.total} | ${
		report.summary.functions.covered
	} | ${report.summary.functions.percentage.toFixed(2)}% |\n`;
	markdown += `| Branches | ${report.summary.branches.total} | ${
	...
	for (const file of sortedFiles) {
		...
		markdown += `| ${relativePath} | ${file.lines.percentage.toFixed(
			2
		)}% | ${file.statements.percentage.toFixed(2)}% | ${file.functions.percentage.toFixed(
			2
