...
<div class="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg" data-testid="interval-selector">
	...
	<div class="flex flex-col gap-2">
		<p class="text-sm text-gray-600 dark:text-gray-400">
			Select the duration multiplier for each phase (standard pattern: x + x + 2x + x + x, where x =
			2 minutes).
		</p>
		...
		<div class="grid grid-cols-5 gap-1 mt-2 text-xs text-center">
			{#each [1, 1, 2, 1, 1] as multiplier, i}
				<div
					class="p-2 rounded bg-primary/10 dark:bg-primary/20 flex flex-col items-center justify-center"
				>
					<span class="font-medium"
						>{i === 0 || i === 4 ? 'Loud' : i === 1 || i === 3 ? 'Whisper' : 'Silent'}</span
					>
					<span>{multiplier * currentMultiplier * 2} min</span>
				</div>
			{/each}
		</div>
